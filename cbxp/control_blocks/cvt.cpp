...
nlohmann::json CVT::get(void* p_control_block, const uint64_t buffer_length) {
  ...
  cvt_json["cvtasvt"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtasvt);
  cvt_json["cvtecvt"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtecvt);
  ...
  cvt_json["cvtabend"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtabend);
  cvt_json["cvtamff"]  = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtamff);
  cvt_json["cvtasmvt"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtasmvt);
  ...
  cvt_json["cvtcsd"]   = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtcsd);
  Logger::getInstance().debug("cvtcsd");
  ...
  Logger::getInstance().debug("cvtdcb");
  cvt_json["cvtdcpa"]  = formatter_.getBitmap<uint32_t>(p_cvtmap->cvtdcpa);
  cvt_json["cvtdfa"]   = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtdfa);
  cvt_json["cvtedat2"] = formatter_.getBitmap<uint32_t>(p_cvtmap->cvtedat2);
  cvt_json["cvteplps"] = formatter_.getHexPtr<uint32_t>(p_cvtvstgx->cvteplps);
  cvt_json["cvtexit"]  = formatter_.getHex<uint16_t>(p_cvtmap->cvtexit);
  cvt_json["cvtexp1"]  = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtexp1);
  ...
  Logger::getInstance().debug("cvtflag7");
  ...
  cvt_json["cvtgda"]   = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtgda);
  cvt_json["cvtgrsst"] = formatter_.getBitmap<uint32_t>(p_cvtmap->cvtgrsst);
  cvt_json["cvtgvt"]   = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtgvt);
  cvt_json["cvthid"]   = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvthid);
  Logger::getInstance().debug("cvthid");
  cvt_json["cvtixavl"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtixavl);
  cvt_json["cvtjesct"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtjesct);
  cvt_json["cvtlccat"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtlccat);
  cvt_json["cvtldto"]  = formatter_.getHex<uint64_t>(p_cvtxtnt2->cvtldto);
  cvt_json["cvtlink"]  = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtlink);
  cvt_json["cvtlso"]   = formatter_.getHex<uint64_t>(p_cvtxtnt2->cvtlso);
  Logger::getInstance().debug("cvtlso");
  cvt_json["cvtmaxmp"] = p_cvtmap->cvtmaxmp;
  cvt_json["cvtmdl"]   = formatter_.getHex<uint16_t>(p_cvtfix->cvtmdl);
  cvt_json["cvtmser"]  = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtmser);
  cvt_json["cvtopctp"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtopctp);
  ...
  cvt_json["cvtpccat"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtpccat);
  cvt_json["cvtpcnvt"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtpcnvt);
  cvt_json["cvtprltv"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtprltv);
  Logger::getInstance().debug("cvtprltv");
  cvt_json["cvtprod"] = formatter_.getHex<uint64_t>(p_cvtfix->cvtprod) +
                        formatter_.getHex<uint64_t>(p_cvtfix->cvtprod + 8);
  cvt_json["cvtpsxm"]  = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtpsxm);
  cvt_json["cvtpvtp"]  = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtpvtp);
  cvt_json["cvtqtd00"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtqtd00);
  cvt_json["cvtqte00"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtqte00);
  cvt_json["cvtrac"]   = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtrac);
  cvt_json["cvtrcep"]  = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtrcep);
  Logger::getInstance().debug("cvtrcep");
  cvt_json["cvtrczrt"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtrczrt);
  ...
  cvt_json["cvtrtmct"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtrtmct);
  cvt_json["cvtsaf"]   = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtsaf);
  cvt_json["cvtscpin"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtscpin);
  Logger::getInstance().debug("cvtscpin");
  ...
  cvt_json["cvtsmca"]  = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtsmca);
  ...
  cvt_json["cvtsvt"]   = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtsvt);
  cvt_json["cvtsysad"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtsysad);
  cvt_json["cvttpc"]   = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvttpc);
  Logger::getInstance().debug("cvttpc");
  cvt_json["cvttvt"]  = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvttvt);
  ...
  Logger::getInstance().debug("cvttz");
  cvt_json["cvtucbsc"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtucbsc);
  cvt_json["cvtundvm"] = formatter_.getBitmap<uint32_t>(p_cvtxtnt2->cvtundvm);
  cvt_json["cvtuser"]  = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtuser);
  cvt_json["cvtverid"] = formatter_.getHex<uint64_t>(p_cvtfix->cvtverid);
  cvt_json["cvtvfget"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtvfget);
  cvt_json["cvtvfind"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtvfind);
  Logger::getInstance().debug("cvtvfind");
  cvt_json["cvtvpsib"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtvpsib);
  cvt_json["cvtvwait"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvtvwait);
  cvt_json["cvt0ef00"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvt0ef00);
  cvt_json["cvt0pt0e"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvt0pt0e);
  cvt_json["cvt0pt02"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvt0pt02);
  cvt_json["cvt0pt03"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvt0pt03);
  cvt_json["cvt0scr1"] = formatter_.getHexPtr<uint32_t>(p_cvtmap->cvt0scr1);
  Logger::getInstance().debug("got it all");
